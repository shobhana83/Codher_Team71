...
Class Action {
	...
	function login2(){
		...
		if($qry->num_rows > 0){
			...
			if($_SESSION['username'] > 0){
				$bio = $this->db->query("SELECT * FROM register where id = ".$_SESSION['login_alumnus_id']);
				if($bio->num_rows > 0){
			...
			if(isset($_SESSION['bio']['status']) && $_SESSION['bio']['status'] != 1){
				...
				//return "Alumnus id : ".$_SESSION['login_alumnus_id'];
				return 1;
		}else{
			return 3;
		}
	...
	function save_user(){
		...
		if(empty($id)){
			...
			//return "insert, chk = $chk";
		}else{
			$save = $this->db->query("UPDATE users set ".$data." where id = ".$id);
			//return "update, chk = $chk";
		}
		if($save){
			return 1;
		}
